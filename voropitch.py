...
try:
    if uploaded_file:
        ...
        if canvas_image.json_data is not None:
            ...
            if n_lines >= 4:
                image.set_info(
                    pd.json_normalize(canvas_image.json_data["objects"]),
                    hlines + vlines,
                )
                ...
                with p_col2:
                    ...
                    original = (
                        True  # st.checkbox('Select on original image', value=True)
                    )
                ...
                if canvas_converted.json_data is not None:
                    if len(canvas_converted.json_data["objects"]) > 0:
                        dfCoords = pd.json_normalize(
                            canvas_converted.json_data["objects"]
                        )
                        if original:
                            dfCoords["x"] = (
                                dfCoords["left"]
                            ...
                            dfCoords["y"] = (
                                dfCoords["top"]
                                + dfCoords["height"] * dfCoords["scaleY"]
                            )
                            dfCoords[["x", "y"]] = image.h.apply_to_points(
                                dfCoords[["x", "y"]].values
                            )
                        else:
                            dfCoords["x"] = (
                                dfCoords["left"]
                                + dfCoords["width"] * dfCoords["scaleX"]
                            )
                            dfCoords["y"] = (
                                dfCoords["top"]
                                + dfCoords["height"] * dfCoords["scaleY"]
                        ...
                        dfCoords[["x", "y"]] = (
                            dfCoords[["x", "y"]] / image.h.coord_converter
                        )
                        dfCoords["team"] = dfCoords.apply(
                            lambda x: {
                                code: color for color, code in colors.items()
                            }.get(x["stroke"]),
                            axis=1,
                        )
                    ...
                    sensitivity = int(
                        st.slider(
                            "Sensitivity (decrease if it is drawing over the players; "
                    ...
                    with o_col2:
                        ...
                        voronoi_opacity = int(
                            st.slider("Voronoi Opacity", 0, 100, value=20) * 2.5
                        )
                        player_highlights = st.multiselect(
                            "Players to highlight", list(dfCoords.index + 1)
                        )
                        player_size = st.slider("Circle size", 1, 10, value=2)
                        player_opacity = int(
                            st.slider("Circle Opacity", 0, 100, value=50) * 2.5
                        )
                    with o_col1:
                    ...
                    with o_col3:
                        ...
                        for pid, coord in dfCoords.iterrows():
                            ...
                            draw.draw_text(
                                coord[["x", "y"]] + 0.5, f"{pid}", coord["team"]
                            )
                        st.image(draw.compose_image(sensitivity))

                    st.markdown(
                        get_table_download_link(df_output), unsafe_allow_html=True
                    )

                    # mplsoccer based output viz
                    _, mpl_output, _ = st.columns(
                        (1, 2, 1)
                    )  # temporary, as fig size doesn't seem to be working
