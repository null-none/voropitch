            ...
            if canvas_converted.json_data is not None:
                if len(canvas_converted.json_data["objects"]) > 0:
                    ...
                    dfCoords["team"] = dfCoords.apply(
                        lambda x: {code: color for color, code in colors.items()}.get(
                            x["stroke"]
                        ),
                        axis=1,
                    )
                ...
                with o_col2:
                    ...
                    player_highlights = st.multiselect(
                        "Players to highlight", list(dfCoords.index)
                    )
                ...
                with o_col3:
                    ...
                    for pid, coord in dfCoords.iterrows():
                        draw.draw_circle(
                            coord[["x", "y"]].values, coord["team"], 2, player_opacity
                        )
                        draw.draw_text(coord[["x", "y"]] + 0.5, f"{pid}", coord["team"])
                    st.image(draw.compose_image(sensitivity))

                st.markdown(
                    get_table_download_link(dfCoords[["team", "x", "y"]]),
